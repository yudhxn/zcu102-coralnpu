...
### 2026-07-27 (월, 이어서) — 데모 업그레이드: NPU로 신경망 완전연결층 추론 🧠✅

[목표] scalar(x3) 데모를 신경망 한 층 Y = ReLU(W·X + b) 계산으로 업그레이드

[프로그램 설계]
- 4x4 행렬 W · 입력벡터 X(4) · 편향 b(4) → 출력 Y(4), 완전연결층(뉴런 4개) 1개
- DTCM 배치: W +0x00(16) / X +0x40(4) / b +0x50(4) / Y +0x60(4) / done +0x70
- RISC-V로 이중 반복문(행 i, 열 j) + MAC 누적 + 편향 + ReLU 구현

[검증 방식]
- 로컬에 keystone/unicorn 설치 시도 → keystone은 RISC-V 미지원
- 직접 만든 미니 어셈블러로 기계어 생성 + unicorn(RV32) 에뮬레이터로 실행 검증
- 3개 테스트 케이스 모두 기대값과 일치 확인 후 보드 적용

[핵심 발견 — mul(하드웨어 곱셈) 미동작]  ⚠️ 분석 자료
- 1차 버전: `mul` 명령 사용 → 보드에서 헤더 출력 후 **멈춤**(done 미설정)
  · 어제 성공한 x3는 shift+add였고 mul 미사용 → 정상이었음
  · 즉 현재 합성된 CoreMiniAxi 구성은 M-확장 곱셈 실행 시 코어가 정지/폴트
- 2차 버전: 곱셈을 **shift+add 소프트웨어 루프**로 구현(mul 제거) → 정상 동작
- README 리스크("매트릭스 연산이 DSP 대신 LUT, 곱셈 경로 불확실")가 실측으로 확인됨
  → M6 한계 분석의 근거: 축소 합성 구성에서 HW 곱셈기 부재 → SW 우회 필요

[결과]
- W·X+b, ReLU까지 NPU가 계산: 예) X=[3,1,2,5] → Y=[16,0,10,113] (2행은 -45→ReLU→0)
- A53가 계산한 기대값과 4개 원소 전부 일치 (PASS)
- **Coral NPU(RISC-V)가 신경망 한 층의 추론을 실제 수행** → 데모 업그레이드 완료

[다음]
1. FC 버전으로 BOOT.bin 재생성 → SD 데모 갱신
2. M6: 실행 사이클 · 리소스 사용량 측정, mul 미지원 등 한계 정리

### 2026-07-27 (월, 정정) — mul(M-확장) 실측 재검증: 정상 동작으로 확인
[정정] 앞 항목의 "mul 미지원" 판단을 정정한다.
[통제 실험] NPU에 트랩 핸들러(mtvec)를 걸고 mul 실행 → 예외 없이 정상, 6×7=42 (done=1).
[FC 재실행] mul 버전 FC(반복문 내 16회 곱셈) 재실행 → Y=[16,0,10,113] 정상.
[결론] M-확장·곱셈기는 정상 동작. 앞선 FC(mul) 멈춤은 하드웨어 한계가 아니라 일시적 실행 상태(직전 디버그 세션 미종료 추정)였음.
[영향] 곱셈은 DSP 아닌 LUT로 구현(면적·속도 특성)일 뿐 기능은 정상. 이후 모델은 mul을 그대로 사용 가능.

### 2026-07-27 (월) — 모델 확장: 2층 신경망(2→2→1, ReLU)으로 XOR 해결
[의미] 단일 FC층에서 은닉층 있는 2층 신경망으로 확장. "은닉층이 왜 필요한가"의 고전 예제 XOR.
[구성] W1=[[1,1],[1,1]] b1=[0,-1] / W2=[1,-2] b2=0. hidden=ReLU(W1·X+b1), out=W2·hidden+b2.
[검증] 직접 만든 어셈블러 + unicorn 에뮬레이터로 4입력 전부 통과. mul 사용(정상 확인됨).
