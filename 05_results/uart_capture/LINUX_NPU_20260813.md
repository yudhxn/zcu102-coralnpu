...
===== MNIST 28x28 on Coral NPU (FC 784-32-10 int8) =====
weights byte-packed: 25.4KB / DTCM 32KB   int8 acc 9641/10000 (emu)
   1000 / 10000 ...  acc 0.9660
   ...
  10000 / 10000 ...  acc 0.9641
===== full: 9641 / 10000 correct (96.41%)  [22.3s, 2.23ms/장] =====
expected(emulator): 9641 / 10000
== HW/EMU 완전 일치 — PASS ==
```

**10,000장 전수에서 에뮬레이터 기대값과 한 장도 어긋나지 않았다.**
장당 2.23ms, 전수 22.3초. 스칼라 코어 50MHz 기준.

> **측정 주의** — 처음에는 `clock()`으로 시간을 재서 "장당 0.05ms"가 나왔다.
> `clock()`은 CPU 시간만 세는데 이 프로그램은 NPU 완료를 기다리며 잠들어 있어
> 대기 구간이 통째로 빠진 값이었다. 25,088 MAC 를 2,500 사이클에 처리한 셈이라
> 물리적으로 불가능한 수치. `clock_gettime(CLOCK_MONOTONIC)` 으로 고치고
> 폴링도 바쁜 대기로 바꿔 실측한 결과가 위 2.23ms 이며, 사이클당 약 0.22 MAC 로
> 스칼라 코어에 타당하다.
...
| **리눅스 전수 (2026-08-13)** | **실제 보드** | **9,641 / 10,000 — 에뮬과 완전 일치** |
