# =============================================================================
#  rebuild_project.tcl  —  Coral NPU on ZCU102, Vivado 프로젝트 완전 재건
# =============================================================================
#
#  목적
#    SSD 고장으로 날아간 Vivado 프로젝트를, GitHub에 남은 RTL 소스와
#    docs/LOG.md 기록(M4, 2026-07-21 / 2026-07-23)만으로 그대로 되살린다.
#
...
set PROJ_DIR  "C:/Users/ehdgn/SOTA/zcutonpu/01_hw/coral_zcu102"
set PROJ_NAME "coral_zcu102"
...
#
#    highmem 버전(CoreMiniHighmemAxi.sv / coral_axi_wrapper_highmem.sv)은
#    LeNet-5 같은 큰 모델용이다. 지금 데모(MNIST/CNN)에는 불필요하므로 제외.
# ---------------------------------------------------------------------------
add_files -norecurse [list \
    "$RTL_DIR/CoreMiniAxi.sv" \
    "$RTL_DIR/coral_axi_wrapper.sv" \
]
set_property file_type SystemVerilog [get_files *.sv]
update_compile_order -fileset sources_1
puts "\[OK\] RTL 2개 추가 (ClockGate/RstSync는 CoreMiniAxi.sv 내장이라 제외)"
...
# ★ 0x5_0000_0000 은 HPM1_FPD 의 주소창이다. HPM0만 쓰면 주소 배정이 실패한다.
...
connect_bd_intf_net [get_bd_intf_pins zynq_ultra_ps_e_0/M_AXI_HPM0_FPD] \
                    [get_bd_intf_pins axi_smc_ctrl/S01_AXI]
...
# ★ 마스터(m_axi) 세그먼트까지 0x5_0000_0000 으로 밀면 서로 겹쳐 validate 실패.
#   PS->Coral 슬레이브 창 하나만 옮긴다.
set coral_segs [get_bd_addr_segs -quiet coral_axi_wrapper_0/s_axi/reg0]
...
puts "\n>>> 타이밍 WNS = $wns ns   (2026-07-23 기록: +0.462)"
if {$wns < 0} {
    puts "\[경고\] 타이밍 위반. PL_CLK0을 더 낮추세요 (예: 50 -> 40MHz)."
}

# ---------------------------------------------------------------------------
# 11. XSA 내보내기 (비트스트림 포함)
# ---------------------------------------------------------------------------
set xsa_path "${PROJ_DIR}/${BD_NAME}_wrapper.xsa"
...
puts "   타이밍 WNS : $wns ns"
puts ""
puts " 다음 단계 -> 01_hw/REBUILD.md 의 'B. Vitis' 부터"
puts "=============================================================="
